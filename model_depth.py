...
class ParsingNet(nn.Module):
                ...
                for j in range(len(cur_DecoderInputIndex)):
                    if stacks[-1] != "__StackRoot__":
                        ...
                        if len(stack_head) < 3:
                            ...
                            input_left = cur_EncoderOutputs[
                                cur_ParsingIndex[j]
                            ].unsqueeze(0)
                            input_right = cur_EncoderOutputs[stack_head[-1]].unsqueeze(
                                0
                            )
                            ...
                            if config.average_edu_level is True:
                                input_left = torch.mean(
                                    cur_EncoderOutputs[
                                        stack_head[0] : cur_ParsingIndex[j] + 1, :
                                    ],
                                    keepdim=True,
                                    dim=0,
                                )
                                input_right = torch.mean(
                                    cur_EncoderOutputs[
                                        cur_ParsingIndex[j] + 1 : stack_head[-1] + 1, :
                                    ],
                                    keepdim=True,
                                    dim=0,
                                )
                                ...
                                input_right = cur_EncoderOutputs[
                                    stack_head[-1]
                                ].unsqueeze(0)
                            ...
                            # Stacks stuff
                            stack_left = stack_head[
                                : (cur_ParsingIndex[j] - stack_head[0] + 1)
                            ]
                            stack_right = stack_head[
                                (cur_ParsingIndex[j] - stack_head[0] + 1) :
                            ]
                            del stacks[-1]
                            Loop_label_batch = Loop_label_batch + 1
    ...
    def TestingLoss(
        ...
        return (
            Loss_tree_batch,
            Loss_label_batch,
            (SPAN_batch if GenerateTree else None),
